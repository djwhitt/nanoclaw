...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  if (!isMainGroup && group.requiresTrigger !== false) {
    if (!hasTriggerMessage(chatJid, group, missedMessages)) return true;
  }

  const prompt = formatMessages(missedMessages, TIMEZONE);
  const attachments = collectAttachments(missedMessages);
...
async function runAgent(
  ...
  writeTasksSnapshot(group.folder, isMain, toTaskSnapshotRows(tasks));
    ...
    try {
      ...
      if (messages.length > 0) {
        ...
        for (const [chatJid, groupMessages] of messagesByGroup) {
          ...
          if (needsTrigger) {
            if (!hasTriggerMessage(chatJid, group, groupMessages)) continue;
          }
...
async function main(): Promise<void> {
  ...
  startIpcWatcher({
    ...
    sendFile: (jid, filePath, caption) =>
      routeFile(channels, jid, filePath, caption),
    sendComponents: (jid, text, components) => {
      const channel = findChannel(channels, jid);
      if (!channel) throw new Error(`No channel for JID: ${jid}`);
      if (!channel.sendComponents)
        throw new Error(`Channel ${channel.name} does not support components`);
      return channel.sendComponents(jid, text, components);
    },
    updateComponents: (jid, messageId, text, components) => {
      const channel = findChannel(channels, jid);
      if (!channel) throw new Error(`No channel for JID: ${jid}`);
      if (!channel.updateComponents)
        throw new Error(
          `Channel ${channel.name} does not support updateComponents`,
        );
    ...
    writeGroupsSnapshot: (gf, im, ag, rj) =>
      writeGroupsSnapshot(gf, im, ag, rj),
    onTasksChanged: () => {
      const taskRows = toTaskSnapshotRows(getAllTasks());
